...
def std_order_feature(df_buy, time_freq, rolling_freq):
    ...
    results = rolling_diff # MODIFIED
    return results


def avg_order_feature(df_buy, time_freq, rolling_freq):
    df_buy = df_buy.groupby(df_buy.index).count()
    df_buy[df_buy == 1] = 0
    df_buy[df_buy > 1] = 1
    buy_volume = df_buy.groupby(pd.Grouper(freq=time_freq))['btc_volume'].sum()
...
def build_features(file, coin, time_freq, rolling_freq, index):
    ...
    results_df = pd.DataFrame(
        {'date': date,
         'pump_index': index,
         'std_order': std_order_feature(df_buy, time_freq, rolling_freq).values,
         'avg_order': avg_order_feature(df_buy, time_freq, rolling_freq).values,
...
def build_features_multi(time_freq, rolling_freq):
    ...
    for f in files:
        ...
        results_df = results_df[(results_df['date'] >= date_datetime - datetime.timedelta(hours=1)) & (results_df['date'] <= date_datetime)]

        all_results_df = pd.concat([all_results_df, results_df])
        count += 1

    all_results_df.to_csv('features/features_{}_1.csv'.format(time_freq), index=False) # MODIFIED


def compute_features():
    build_features_multi(time_freq='60S', rolling_freq=500)
